...
describe('Workflow e2e', () => {
  ...
  it('should run full OS workflow with real servico/peca data', async () => {
    // create part
    const pecaRes = await request(app.getHttpServer()).post('/pecas').send({ nome: 'Filtro', preco: 50, estoque: 10 });
    okStatus(pecaRes);
    const pecaId = pecaRes.body.id;
    expect(pecaId).toBeDefined();

    // create service
    const servRes = await request(app.getHttpServer()).post('/servicos').send({ nome: 'Troca óleo', preco: 120 });
    okStatus(servRes);
    ...
    const diagRes = await request(app.getHttpServer()).post(`/os/${osId}/diagnostico`).send();
    okStatus(diagRes);
    ...
    // gerar orcamento
    const gerarRes = await request(app.getHttpServer()).post(`/os/${osId}/orcamento`).send();
    okStatus(gerarRes);

    // aprovar
    const aprovarRes = await request(app.getHttpServer()).post(`/os/${osId}/aprovar`).send();
    okStatus(aprovarRes);

    // iniciar execucao
    const executarRes = await request(app.getHttpServer()).post(`/os/${osId}/executar`).send();
    okStatus(executarRes);

    // finalizar
    const finalizarRes = await request(app.getHttpServer()).post(`/os/${osId}/finalizar`).send();
    okStatus(finalizarRes);

    // entregar
    const entregarRes = await request(app.getHttpServer()).post(`/os/${osId}/entregar`).send();
    okStatus(entregarRes);

    // fetch and verify OS exists
    const fetchRes = await request(app.getHttpServer()).get(`/os/${osId}`).send();
    okStatus(fetchRes);
